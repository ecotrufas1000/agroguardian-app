...
def obtener_datos_clima():
    ...
    try:
        ...
        return {
            ...
            "nubes": r['clouds']['all'],
            "clima_principal": r['weather'][0]['main'],
            "desc": r['weather'][0]['description'].capitalize()
        }
    except:
        return {"temp": 28, "hum": 60, "presion": 1013, "nubes": 0, "clima_principal": "Clear", "desc": "Cielo despejado"}
...
def obtener_pronostico_semanal():
    ...
    for i in range(1, 8):
        ...
        t_min = t_max - 11.5 if i < 5 else -2.0
        pronostico.append({
...
with st.sidebar:
    ...
    st.markdown("---")
    # Agregamos "Alerta de Granizo" al menú
    seleccion = st.radio("Menú de Navegación:", ["📊 Monitoreo de Lote", "💧 Balance Hídrico", "❄️ Pronóstico de Heladas", "⛈️ Alerta de Granizo"])
    st.markdown("---")
...
if seleccion == "📊 Monitoreo de Lote":
    ...
    st.title("❄️ Pronóstico de Heladas")
...
# --- NUEVA PÁGINA 4: ALERTA DE GRANIZO ---
elif seleccion == "⛈️ Alerta de Granizo":
    st.title("⛈️ Probabilidad de Granizo en Lote")
    st.info("Estimación científica basada en inestabilidad atmosférica y radar Doppler.")
    
    clima = obtener_datos_clima()
    
    # Motor de Probabilidad AgroGuardian
    prob = 0
    if clima['temp'] > 26: prob += 20
    if clima['hum'] > 75: prob += 20
    if clima['presion'] < 1010: prob += 30
    if clima['nubes'] > 80: prob += 15
    if clima['clima_principal'] == "Thunderstorm": prob += 15

    # Visualización de Riesgo
    col_p1, col_p2 = st.columns([1, 2])
    
    with col_p1:
        color_alert = "green" if prob < 40 else "orange" if prob < 75 else "red"
        st.markdown(f"""<div style="text-align: center; border: 4px solid {color_alert}; padding: 30px; border_radius: 50%; width: 200px; height: 200px;">
            <p style="margin: 0; color: gray;">RIESGO</p>
            <h1 style="margin: 0; font-size: 60px; color: {color_alert};">{prob}%</h1>
        </div>""", unsafe_allow_html=True)
    
    with col_p2:
        st.write("### 📊 Variables de Inestabilidad")
        st.write(f"- **Presión Atmosférica:** {clima['presion']} hPa (Indicador de celdas)")
        st.write(f"- **Cobertura Nubosa:** {clima['nubes']}% (Desarrollo vertical)")
        st.write(f"- **Estado Actual:** {clima['desc']}")
        
        if prob >= 75:
            st.error("🚨 **ALERTA ROJA:** Condiciones altamente compatibles con granizo. Resguarde equipos.")
        elif prob >= 40:
            st.warning("⚠️ **ATENCIÓN:** Atmósfera inestable. Probables chaparrones fuertes.")
        else:
            st.success("✅ **ESTABLE:** No se detectan indicadores de granizo inminente.")

    st.divider()
    st.write("### 🛰️ Radar Doppler en Tiempo Real")
    
    # URL optimizada para embeber (usamos el subdominio de widgets de Windy)
    radar_url = f"https://embed.windy.com/embed2.html?lat={LAT}&lon={LON}&detailLat={LAT}&detailLon={LON}&width=650&height=450&zoom=8&level=surface&overlay=radar&product=radar&menu=&message=&marker=&calendar=now&pressure=&type=map&location=coordinates&detail=&metricWind=default&metricTemp=default&radarRange=-1"
    
    try:
        # Intentamos cargar el iframe
        st.components.v1.iframe(radar_url, height=500, scrolling=False)
    except:
        st.warning("⚠️ El navegador bloqueó el mapa dinámico.")
    
    # Botón de respaldo (Si Firefox bloquea el iframe, esto siempre funciona)
    st.markdown(f"""
        <a href="https://www.windy.com/-Weather-radar-radar?radar,{LAT},{LON},9" target="_blank">
            <button style="width:100%; background-color:#1c1f26; color:white; border:1px solid #2d323e; padding:10px; border-radius:10px; cursor:pointer;">
                🌐 Abrir Radar en Ventana Nueva (Alternativa Firefox)
            </button>
        </a>
    """, unsafe_allow_html=True)
    
    st.caption("Nota: Si no ves el mapa, haz clic en el escudo que aparece en la barra de direcciones de Firefox y selecciona 'Desactivar protección para este sitio'.")