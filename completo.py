...
# ==========================================================
    ...
    _RADIO_KM   = 150  # Radio fijo

    # ── GeoJSON provincias embebido ───────────────────────────
    ...
    # ── Crear raster PNG con máscara circular ─────────────────
    ...
    # ── Leyenda PNG ────────────────────────────────────────────
    def _crear_leyenda_img(vmin, vmax):
        fig, ax = plt.subplots(figsize=(1.1, 4.5))
        norm = mcolors.TwoSlopeNorm(vmin=vmin, vcenter=0, vmax=vmax)
        cb = matplotlib.colorbar.ColorbarBase(ax, cmap=CMAP_BAL, norm=norm, orientation="vertical")
        cb.set_label("mm", fontsize=11)
        cb.ax.tick_params(labelsize=9)
        ax.set_title("Balance\nhídrico", fontsize=9, pad=6)
    ...
    if st.button("🌍 Generar balance hídrico", type="primary", use_container_width=True, key="bh_generar"):
        ...
        tab1, tab2 = st.tabs([
            f"📅 Balance diario  ({fecha_fin_str})",
            f"📆 Balance acumulado  ({acum_dias} días)"
        ])
        ...
        with tab1:
            ...
            m1, vmin1, vmax1 = res1
            col_mapa1, col_ley1 = st.columns([6, 1])
            with col_mapa1:
                folium_static(m1, width=820, height=520)
            with col_ley1:
                st.image(_crear_leyenda_img(vmin1, vmax1), use_container_width=True)
        ...
        with tab2:
            ...
            m2, vmin2, vmax2 = res2
            col_mapa2, col_ley2 = st.columns([6, 1])
            with col_mapa2:
                folium_static(m2, width=820, height=520)
            with col_ley2:
                st.image(_crear_leyenda_img(vmin2, vmax2), use_container_width=True)
